...
| [D016](D016.md) | FINDINGS.md "reformulating hard_subset" entries (2026-09-22/23); human-approved 2026-09-23 ("是的,起草这个D") | **OPEN** — does real difficulty exist outside the 65 structural pairs? Extends trained two-logit-restricted accuracy to all ~666 pairs by reusing D009/D010's saved checkpoints (inference only, no retraining) if they still exist (S0 gates this); extends D008's tensor proxy the same way; correlates the two — the rigorous, full-scale version of the 2026-09-22 proxy-vs-trained check | REVIEW (not yet posted) | D007 (CLOSED), D008 (CLOSED), D009 (CLOSED), D010 (CLOSED), D011 (CLOSED), D015 (CLOSED) | `results/D016/` (pending), `D016.md` `## D` |
