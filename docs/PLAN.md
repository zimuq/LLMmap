...
| [D012](D012.md) | `DECISIONS.md` C1 (γ ablation, never run for `JointGreedy`); human-approved 2026-09-16, sharpened from a broader "k and γ sweep" | **OPEN** 2026-09-16 — does CVaR-weighting matter for `JointGreedy`, or does the joint statistic already implicitly favor hard pairs under mean aggregation? Pre-registered hypothesis: the latter. Reuses D010's own γ=0.1 result; runs γ ∈ {1.0, 0.25, 0.05} through D009's exact trained-pipeline protocol | REVIEW (not yet planned) | D007 (CLOSED), D008 (CLOSED), D009 (CLOSED), D010 (CLOSED) | `results/D012/`, `D012.md` `## R` |
