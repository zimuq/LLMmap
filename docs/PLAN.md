...
| [D016](D016.md) | FINDINGS.md "reformulating hard_subset" entries (2026-09-22/23); human-approved 2026-09-23 ("是的,起草这个D") | **OPEN — P1 approved w/ amendments 2026-09-23.** S0: only seed-0 checkpoints were ever saved (`if r==0` in the training scripts, not deletion) — Call 1 measured retraining as bit-exact (abs_diff 0.0) at 2 min for the 12 missing seeds, approved. S3 (tensor proxy, all 666 pairs) done: structural pairs genuinely harder on average (.816 vs .874) but 4-5 non-structural pairs tie with the easier structural pairs at the tail, all cross-family — a naming typo in the writeup corrected in Review. S1/S2/S4 (the real trained-classifier version) awaiting execution | REVIEW (P1 approved w/ amendments) | D007 (CLOSED), D008 (CLOSED), D009 (CLOSED), D010 (CLOSED), D011 (CLOSED), D015 (CLOSED) | `results/D016/`, `D016.md` `## P` |
