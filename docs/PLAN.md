...
| [D014](D014.md) | D013 R2 + Review (γ=0.5 anomaly on one chain); D012 addendum (JointGreedy never run between γ=0.25 and 1.0); human-approved 2026-09-19, scope: both algorithms | **OPEN** — P1 reviewed and APPROVED WITH AMENDMENTS 2026-09-19. Is γ=0.5's failure a threshold in γ (H1) or an unlucky nested chain (H2)? Fine γ grid trained for both algorithms + a chain-stability arm (30 random half-subsamples of the build configs). S1 found chains recur as *families* across neighbouring γ, so the trained grid alone cannot separate H1 from H2 — **Arm B decides**, under a paired numeric rule fixed before it runs; cross-algorithm agreement is only partly independent (shared tensor). Holds the `METHOD.md §5.2` wording decision. TACC executing S2/S3 | REVIEW (P1 approved w/ amendments) | D007–D013 (all CLOSED) | `results/D014/`, `D014.md` `## R` |
