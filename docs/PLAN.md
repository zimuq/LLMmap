...
|---|---|---|---|---|---|
| [D001](D001.md) | TODO T0.1, T0.2 | OPEN — R posted, held open pending human call on R6 (see D001 `## Review`) | REVIEW (P1 approved w/ amendments) | none | `results/D001/`, `D001.md` `## R` |
...
- **R6 remeasurement** (I3-compliant, non-saturating separability statistic,
  small pilot) — recommended in `D001.md` `## R` §R6. Not drafted as a D yet;
  pending the human's call (see D001's post-results `## Review` entry,
  "Needs your input"). Working id if approved: **D004**.
...
- TACC executes only `status: OPEN` D's whose `gate` is satisfied — `AUTO`
  ...
  verdict (D001 and D002 both have one now; D003 does not yet have a `## P`
  to review).
