...
| [D017](D017.md) | Human proposal, this session (2026-09-24); human-approved ("先起草D017") | **OPEN** — does joint energy's advantage (aggregate and hard-pair-specific) survive a linear classifier instead of the paper's attention network, on the exact same frozen traces/splits/selected chains? Three symmetric pre-registered outcomes (classifier-agnostic / classifier-dependent / falsified), all equally informative. Explicitly does not reopen the hard-pair-definition question (parked) — uses only the existing 65-pair `hard_subset` and the two named pairs | REVIEW (not yet posted) | D007 (CLOSED), D008 (CLOSED), D009 (CLOSED), D010 (CLOSED), D015 (CLOSED), D016 (CLOSED) | `results/D017/` (pending), `D017.md` `## D` |
