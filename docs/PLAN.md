...
| [D018](D018.md) | Un-shelved "Direction B" (`PLAN.md` "Not yet a D"); human-approved 2026-09-26 (accepts cost, both classifiers) | **OPEN** — runs Algorithm H.1's real classifier-in-the-loop greedy search on our own 259-query pool (not the paper's 50), under both the attention network and the linear classifier, to test the strongest possible version of DECISIONS.md D5 claim (2): does our classifier-free selection match/beat real classifier-in-the-loop search head-to-head, not just beat `paper8`'s published output? Mandatory cost pilot (S0) and three design Calls (split protocol, seed policy, pooling) before any full-scale run. Three symmetric pre-registered outcomes | REVIEW (not yet posted) | D007 (CLOSED), D008 (CLOSED), D009 (CLOSED), D010 (CLOSED), D015 (CLOSED), D017 (CLOSED) | `results/D018/` (pending), `D018.md` `## D` |
