...
| [D017](D017.md) | Human proposal, this session (2026-09-24); human-approved ("先起草D017") | **OPEN — P1 approved w/ amendments 2026-09-25.** Call 1 reversed my mean-pool default to concatenation (verified: an apparent order-dependence was a solver-convergence artifact, closes to exactly 0 with tighter settings) — concat primary, mean-pool secondary. **Notable, non-blocking finding: the linear classifier beats the paper's attention network by +6.8pp** (0.9027 vs 0.8346, `paper8` `k`=8, same chain) — D017's "simpler/weaker classifier" framing no longer fits at this sample size (2,775 traces vs. ~3.0M params); flagged for the human, not investigated further here. S2–S5 (the actual question) awaiting execution | REVIEW (P1 approved w/ amendments) | D007 (CLOSED), D008 (CLOSED), D009 (CLOSED), D010 (CLOSED), D015 (CLOSED), D016 (CLOSED) | `results/D017/`, `D017.md` `## P` |
