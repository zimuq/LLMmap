...
| [D015](D015.md) | `docs/REVIEW-2026-09-22-hard-pairs.md` §8/§10.A; human-approved 2026-09-22 | **OPEN** — per-pair decomposition of `hard_subset` (65 structural pairs) for paper8/coverage/joint energy at `k`=1,8; read-only over already-saved `per_hard_pair` arrays, zero new training/selection (I6) | REVIEW (not yet posted) | D007 (CLOSED), D009 (CLOSED), D010 (CLOSED), D012 (CLOSED), D013 (CLOSED) | `results/D015/` (pending), `D015.md` `## D` |
